    ...
    ...)
{
    ...
    if (!is.null(labpos.left)) {
        ...
        text(1-offset.lab.l, bump_overlaps(leftlabs[,1], decimals = decimals),
             col=col.lab[which(!is.na(data[,1]))], rownames(leftlabs), pos=labpos.left, 
             cex=cex.lab, font=font.lab, family=family, xpd=NA, offset=offset.down)
    }
    # right-side labels
    if (!is.null(labpos.right)) {
        rightlabs <- data[!is.na(data[,ncol(data)]), ncol(data), drop = FALSE]
        text(ncol(data)+offset.lab.r, bump_overlaps(rightlabs[,1], decimals = decimals), 
             col=col.lab[which(!is.na(data[,ncol(data)]))], rownames(rightlabs), pos=labpos.right, 
             cex=cex.lab, font=font.lab, family=family, xpd=NA)
    }
