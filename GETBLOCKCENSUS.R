...
tmp.st <- jsonlite::fromJSON(paste0("https://api.census.gov/data/2010/dec/sf1?get=P001001,P003001,P003002,P003003,P010001,P010002,P010003,P010004,P005001,P005002,P005003,P005004,P006001,P006002,P006003,P007001,P007002,P007003,P007004,P042002,P042003,P042004,NAME,PLACE,BLKGRP&for=block:*&in=state:", unique(cnty[,"state"])[j], "%20county:",st[i,"county"]))
	write.csv(tmp.st, paste0(unique(cnty[,"state"])[j], st[i,"county"], ".csv"), row.names=F)
}}