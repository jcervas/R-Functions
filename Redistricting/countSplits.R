countSplits <- function(plan = NULL, census_blocks = NULL, geo = "COUNTY", custom_geo = NULL, plan_id="GEOID20", block_id="GEOID20", custom_geo_id="GEOID20") {
  ...
  if (exists("custom_geo")) {
    ...
    colnames(plan_tmp)[colnames(plan_tmp) %in% max(colnames(plan_tmp))] <- "geo"
  } else {
...
# countSplits(plan = plan, census_blocks = census_blocks, custom_geo = NULL, geo = "COUNTY", plan_id="GEOID20", block_id="GEOID20", custom_geo_id=NULL)
